...
def _validate_draft_content(subject: str, body: str) -> None:
    """Minimal sanity check. Heavy semantic correctness (is this a real email
    or instruction fragment?) is the LLM planner's job — its system prompt
    has explicit guidance and a worked example. This validator only catches
    the trivially-broken case where subject and body are identical, which
    is never a useful email regardless of model strength."""
