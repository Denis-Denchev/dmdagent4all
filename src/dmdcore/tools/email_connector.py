...
def _draft_from_args(args: dict[str, Any], settings: EmailSettings) -> dict[str, Any]:
    ...
    _validate_draft_content(subject, body)
    return {
        "provider": settings.provider,
        "from": settings.from_addr,
        "to": to,
...
def _validate_draft_content(subject: str, body: str) -> None:
    subject_clean = subject.strip()
    body_clean = body.strip()
    if subject_clean.casefold() == body_clean.casefold():
        raise ValueError(
            "Email subject is identical to the body. "
            "The subject must summarize the message; the body must be the actual content."
        )
    instruction_markers = (
        "generated by you",
        "generated by ai",
    ...
    body_lower = body_clean.casefold()
    if any(marker in body_lower for marker in instruction_markers) and len(body_clean.split()) < 12:
        raise ValueError(
            "Email body looks like a fragment of the user's instruction rather than a written email. "
            "Re-read what the user wants you to say to the recipient, write that as a real message, "
            "and only mention 'generated by the assistant' as a closing note."
        )
