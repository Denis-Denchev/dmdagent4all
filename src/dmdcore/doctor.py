...
from dmdcore.permissions import RiskLevel
from dmdcore.sandbox.terminal import ALWAYS_BLOCKED, BLOCKED_ARG_FRAGMENTS, TerminalPolicy
        ...
        if path.exists() and path.is_dir() and os.access(path, os.W_OK):
            ...
            checks.append(
                DoctorCheck(
                    ...
                    "Run `dmdcore init` and check filesystem permissions.",
                )
            )
    return checks
...
def _check_ollama(base_url: str, model: str) -> DoctorCheck:
    ...
    return DoctorCheck(
        ...
        f"Run `ollama pull {model}`." if model else "Set a local model with `dmdcore models set <model>`.",
    )
        ...
        if not tool_enabled:
            checks.append(
                DoctorCheck(
                    ...
                    "Run `dmdcore tools enable terminal.run` when you are ready.",
                )
            )
        if "terminal.run" not in permissions:
            checks.append(
                DoctorCheck(
                    ...
                    "Run `dmdcore permissions grant terminal.run` after reviewing the allowlist.",
                )
            )
    else:
        ...
        if tool_enabled:
            checks.append(
                DoctorCheck(
                    ...
                    "Run `dmdcore terminal enable` or `dmdcore tools disable terminal.run`.",
                )
            )
        ...
        if binary in ALWAYS_BLOCKED:
            checks.append(
                DoctorCheck(
                    ...
                    "Remove it with `dmdcore terminal remove ...`.",
                )
            )
            continue
    ...
    if allowed:
        ...
        checks.append(
            DoctorCheck(
                ...
                "Run `dmdcore telegram allow <telegram_user_id>`.",
            )
        )
    token_env = str(telegram.get("bot_token_env", "DMDCORE_TELEGRAM_BOT_TOKEN"))
