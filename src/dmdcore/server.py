...
from dmdcore.autonomy import local_dev_autonomy_enabled
from dmdcore.config import load_config, update_config, write_default_config
from dmdcore.doctor import doctor_summary, run_doctor
from dmdcore.email_oauth import (
...
from dmdcore.memory.manager import MemoryPathError
from dmdcore.model_presets import MODEL_MODES
...
from dmdcore.runtime import build_agent_core
from dmdcore.sandbox import TerminalPolicy, active_terminal_processes, emergency_stop_terminal_processes
from dmdcore.agent.runtime_state import invalidate_runtime_state_cache
from dmdcore.tools import build_builtin_registry
from dmdcore.tools.base import ToolRuntimeContext
from dmdcore.tools.email_connector import (
    _normalize_app_password,
    _normalize_email_login_value,
    test_email_connection,
)
from dmdcore.tools.storage import downloads_root_from_config
...
from dmdcore.workspace import WorkspaceError, WorkspaceManager


DEFAULT_DEEPSEEK_API_KEY_ENV = "DMDCORE_DEEPSEEK_API_KEY"
...
def create_app() -> FastAPI:
    ...
    def start_background_services() -> None:
        if os.environ.get("DMDCORE_NO_TELEGRAM") != "1":
...
def _dashboard_static_dir() -> Path | None:
    configured = os.environ.get("DMDCORE_STATIC_DIR", "").strip()
    candidates = [
...
class TelegramRuntime:
    ...
    def start(self, config: dict[str, Any]) -> bool:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(target=run, name="dmdcore-api-telegram", daemon=True)
            self._thread.start()
            return True
...
class ReminderRuntime:
    ...
    def start(self) -> None:
        if self._thread is not None and self._thread.is_alive():
            return
        self._stop_event = threading.Event()
        self._thread = threading.Thread(target=self._run, name="dmdcore-reminders", daemon=True)
        self._thread.start()
