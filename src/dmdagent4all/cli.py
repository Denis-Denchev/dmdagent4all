...
from dmdagent4all.doctor import checks_to_json, doctor_summary, run_doctor
...
from dmdagent4all.runtime import build_agent_core, permission_context_from_config
...
def command_start(
    *,
    ...
    if str(config.get("llm", {}).get("model", "")).strip().lower() == "dumb":
        print("Model looks invalid. Recommended fix: dmdagent model fast --pull")
    print("")
    ...
    try:
        ...
        print("- Change model:      dmdagent model fast --pull")
        print("- Check health:      dmdagent doctor")
        print("")
        if open_browser:
            _open_url(ui_url)
...
def command_model(args: argparse.Namespace) -> int:
    ...
    if selection in {"use", "set"}:
        if len(raw_args) < 2:
            print("Model name is required. Example: dmdagent model use qwen3:8b", file=sys.stderr)
            return 1
        return _set_model_name(raw_args[1], pull=pull)

    print("Unknown model command.", file=sys.stderr)
    print("Examples:")
    print("  dmdagent model")
    print("  dmdagent model fast --pull")
    print("  dmdagent model light --pull")
    print("  dmdagent model use qwen3:8b --pull")
    return 1
...
def _run_terminal_onboarding(paths: AppPaths) -> dict[str, Any]:
    ...
    if model_choice == "local-fast":
        ...
        api_key_env = _ask_text(
            ...
            _default_provider_api_key_env(provider) or "DMDAGENT_OPENAI_COMPATIBLE_API_KEY",
        )
...
def _start_telegram_background_if_ready(
    ...
    *,
    ...
    thread = threading.Thread(target=run, name="dmdagent-telegram", daemon=True)
    thread.start()
...
def _print_chat_response(
    ...
    if status == "error":
        ...
        if "Cannot connect to Ollama" in message:
            print("Fix:")
            print("  /model fast --pull")
            print("  or run: dmdagent model fast --pull")
        return
...
def _ensure_dashboard(
    *,
    ...
    if shutil.which("npm") is None:
        raise RuntimeError("npm is not installed. Install Node.js, then run `dmdagent start` again.")
